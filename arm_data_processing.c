...
int borrowFrom(uint16_t op1, uint32_t op2){
    if(op1<op2)
      return 1;
    return 0;          
}
...
int carryFrom(uint16_t op1, uint32_t op2){
    if((int)op1+(int)op2>0xFFFFFFFF)
      return 1;
    return 0;          
}
...
int overflowFrom(uint16_t op1, uint32_t op2, uint32_t res, int isAddition){
    if(isAddition){
    ...
    switch (opcode)?{
        ...
        case RSB:
          ...
          if (S==1 && rd==R15){
            ...
            flags = (flags + ~(get_bit(borrowFrom(shifter_operand,rnVal),0))) << 1;             //C
            
            flags += get_bit(overflowFrom(shifter_operand,rnVal,arm_read_register(p,rd),0),0)); //V
        ...
        case ADD:
          ...
          if (S==1 && rd==R15){
            ...
            flags = (flags + get_bit(carryFrom(shifter_operand,rnVal),0)) << 1;              //C
            
            flags += get_bit(overflowFrom(shifter_operand,rnVal,arm_read_register(p,rd),1),0)); //V
        ...
        case ADC:
          ...
          if (S==1 && rd==R15){
            ...
            flags = (flags + get_bit(carryFrom(shifter_operand+cflag,rnVal),0)) << 1;              //C
            
            flags += get_bit(overflowFrom(shifter_operand+cflag,rnVal,arm_read_register(p,rd),1),0)); //V
        ...
        case RSC:
          ...
          if (S==1 && rd==R15){
            ...
            flags = (flags + ~(get_bit(borrowFrom(shifter_operand - notcflag,rnVal),0))) << 1;             //C
            
            flags += get_bit(overflowFrom(shifter_operand - notcflag,rnVal,arm_read_register(p,rd),0),0)); //V
...
int and(arm_core p, int rd, int rn, int shifter_operand){
    p->registers[rd]=p->registers[rn]&&shifter_operand;
    return 0;
}

int add(arm_core p, int rd, int rn, int shifter_operand){
    p->registers[rd]=p->registers[rn]+shifter_operand;
    return 0;
}

int sub(arm_core p, int rd, int rn, int shifter_operand){
    p->registers[rd]=p->registers[rn]-shifter_operand;
    return 0;
}